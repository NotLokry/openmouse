...
export function renderServiceWorker(version: string, precache: string[]): string {
...
/** Drops the oldest entries once a cache grows past its limit. */
  ...
  const network = fetch(request)
    .then((response) => {
      if (storable(response)) {
        ...
        // Font variants arrive one at a time; keep the cache bounded.
        void trimCache(cacheName, FONT_CACHE_LIMIT);
      }
      return response;
    })
